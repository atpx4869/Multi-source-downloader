...
class BYSourceAPI(BaseSourceAPI):
    """BY 源 API"""
    
    source_type = SourceType.BY
    
    def __init__(self):
        """初始化 BY API"""
        try:
            from sources.by import BYSource
            self.source = BYSource()
    ...
    def search(self, query: str, limit: int = 100) -> SearchResponse:
        ...
        if not self.source:
            response.error = f"BY 源未初始化: {self.init_error}"
            return response
    ...
    def check_health(self) -> SourceHealth:
        ...
        try:
            if not self.source:
                health.error = f"BY 源未初始化: {self.init_error}"
                return health
